...
%token 		<token> TCOMMA TDOT TCOLON TSEMI TRANGE
%token 		<token> TLPAREN TRPAREN TLBRACKET TRBRACKET
%token 		<token> TPLUS TMINUS TMUL TDIV
%token 		<token> TNOT TAND TOR
%token 		<token> TIF TELSE TWHILE TTO TFOR
...
%type 		<stmt> stmt var_decl var_decl_deduce func_decl conditional_if return while for_loop array_add_element /*print_stmt*/
...
for_loop	: TFOR TLPAREN expr TSEMI expr TSEMI expr TRPAREN block {$$ = new harambe::ForLoop($3,$5,$7,$9);}
...
func_decl 	: TDEF ident TLPAREN func_decl_args TRPAREN TCOLON ident block { $$ = new harambe::FunctionDeclaration($7, $2, $4, $8, @$); }
